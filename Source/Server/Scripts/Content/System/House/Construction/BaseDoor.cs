...
namespace Server.Items
{
	...
	public abstract class BaseHouseDoor : BaseDoor, ISecurable
	{
		...
		public BaseHouse FindHouse()
		{
			...
			m_House = BaseHouse.FindHouseAt(loc, Map, ItemData.Height);

			if (m_House == null && Map != null && Map != Map.Internal)
			{
				var sector = Map.GetSector(loc);

				for (var i = 0; i < sector.Multis.Count; ++i)
				{
					if (sector.Multis[i] is BaseHouse house)
					{
						// Check if door is reasonably close to house location
						if (Utility.InRange(loc, house.Location, 30))
						{
							m_House = house;
							break;
						}
