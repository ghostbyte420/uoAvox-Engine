...
namespace Server.Commands
{
	public static class Categorization
	{
		...
		public static void Initialize()
		{
			...
			var matchedTypes = 0;
			var instantiatedTypes = 0;
			var failedInstantiations = 0;
			...
			foreach (var type in types)
			{
				...
				catch (Exception ex)
				{
					...
					Console.WriteLine($"Categorization: Failed to instantiate {type.FullName}: {ex.GetType().Name} - {ex.Message}");
					continue;
				}
				...
				{
					failedInstantiations++;
					Console.WriteLine($"Categorization: Instantiation returned null for {type.FullName}");
					continue;
				}
				...
				{
					Console.WriteLine($"Categorization: Failed to delete {type.FullName}: {ex.GetType().Name} - {ex.Message}");
				}
			}

			Console.WriteLine($"Categorization: Processed {totalTypes} constructable types");
			Console.WriteLine($"Categorization: {matchedTypes} matched to categories");
			Console.WriteLine($"Categorization: {instantiatedTypes} successfully instantiated");
			Console.WriteLine($"Categorization: {failedInstantiations} failed to instantiate");

			// Detailed breakdown
			var itemsTotal = CountTotalObjects(Items);
			var mobilesTotal = CountTotalObjects(Mobiles);

			Console.WriteLine($"Categorization: Items category has {Items.ObjectCount} objects in {Items.NodeCount} subcategories");
			Console.WriteLine($"Categorization:   Total Items (including subcategories): {itemsTotal}");
			Console.WriteLine($"Categorization: Mobiles category has {Mobiles.ObjectCount} objects in {Mobiles.NodeCount} subcategories");
			Console.WriteLine($"Categorization:   Total Mobiles (including subcategories): {mobilesTotal}");
			Console.WriteLine($"Categorization: Grand total objects: {itemsTotal + mobilesTotal} (should equal {instantiatedTypes})");

			CommandSystem.Register("CatStats", AccessLevel.Administrator, CategorizeStats_OnCommand);
		}

		private static int CountTotalObjects(CategoryNode node)
		{
			var count = node.ObjectCount;
		...
		[Usage("CatStats [categoryPath]")]
		[Description("Shows categorization statistics. Optional path like 'Items/Wearables/Weapons' to show specific category.")]
		private static void CategorizeStats_OnCommand(CommandEventArgs e)
		{
			var from = e.Mobile;
			...
			if (e.Length == 0)
			{
				...
				from.SendMessage($"Total Categories: Objects={Objects.Count}, Items={Items.Count}, Mobiles={Mobiles.Count}");
				from.SendMessage($"Items: {Items.ObjectCount} objects, {Items.NodeCount} subcategories");
				from.SendMessage($"Mobiles: {Mobiles.ObjectCount} objects, {Mobiles.NodeCount} subcategories");

				ShowCategoryTree(from, Items, "Items", 0, 2);
				ShowCategoryTree(from, Mobiles, "Mobiles", 0, 2);
			}
			else
			{
				var path = e.ArgString.Split('/', StringSplitOptions.RemoveEmptyEntries);
				var node = FindCategoryByPath(Objects, path);

				if (node == null)
				{
					from.SendMessage($"Category not found: {e.ArgString}");
					return;
				}

				from.SendMessage($"Category: {node.Name}");
				from.SendMessage($"  Objects: {node.ObjectCount}");
				from.SendMessage($"  Subcategories: {node.NodeCount}");
				from.SendMessage($"  Types for matching: {node.Types.Count}");

				if (node.Types.Count > 0)
				{
					from.SendMessage("  Match types:");
					foreach (var t in node.Types.Take(10))
					{
						from.SendMessage($"    - {t.Name} (Abstract: {t.IsAbstract})");
					}
				}

				ShowCategoryTree(from, node, node.Name, 0, 3);
			}
		}

		private static CategoryNode FindCategoryByPath(CategoryNode root, string[] path)
		{
			var current = root;

			foreach (var segment in path)
			{
				current = current.Nodes.FirstOrDefault(n => n.Name.Equals(segment, StringComparison.OrdinalIgnoreCase));
		...
		private static void ShowCategoryTree(Mobile from, CategoryNode node, string prefix, int depth, int maxDepth)
		{
			if (depth >= maxDepth)
				return;

			var indent = new string(' ', depth * 2);
			from.SendMessage($"{indent}{prefix}: {node.ObjectCount} objects, {node.NodeCount} subcategories");

			if (depth < maxDepth - 1)
			{
				foreach (var child in node.Nodes.Take(20))
				{
					ShowCategoryTree(from, child, child.Name, depth + 1, maxDepth);
				}

				if (node.NodeCount > 20)
				{
					from.SendMessage($"{indent}  ... and {node.NodeCount - 20} more");
				}
			}
		}
		...
		private static void AddTypes(Assembly asm, HashSet<Type> types)
		{
			...
			{
				Console.WriteLine($"Categorization: ReflectionTypeLoadException loading types from {asm.FullName}");
				Console.WriteLine($"  Loaded {ex.Types.Count(t => t != null)} types successfully");

				// Add the types that did load
				foreach (var type in ex.Types)
				{
					if (type != null && IsConstructable(type))
					{
						types.Add(type);
					}
				}
			...
			{
				Console.WriteLine($"Categorization: Failed to load types from {asm.FullName}: {ex.Message}");
			}
		}
	...
	public sealed class CategoryNode : ICategoryEntry, IEnumerable<ICategoryEntry>
	{
		...
		public bool IsMatch(Type type)
		{
			...
			{
				if (type == match || match.IsAssignableFrom(type))
				{
					return true;
				}
			}
