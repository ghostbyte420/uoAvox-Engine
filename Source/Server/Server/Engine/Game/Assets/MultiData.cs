...
namespace Server
{
	public static class MultiData
	{
		...
		private static bool LoadUOP()
		{
			...
			do
			{
				...
				do
				{
					...
					var positionpoint = stream.Position;
					...
					try
					{
						...
						if (cmp == 1)
						{
							...
							var destination = destData.AsSpan();

							if (!Compression.Unpack(source, ref destination))
							{
								continue;
							}
						...
						using var fs = new MemoryStream(destData, 0, decompressedSize);
						...
						try
						{
							...
							for (uint i = 0; i < count; i++)
							{
								if (fs.Position + 14 > fs.Length)
								{
									if (Core.Debug)
									{
										Console.WriteLine($"MultiData: Truncated data at entry {i}/{count}, pos={fs.Position}, length={fs.Length}, chunkID={chunkID}");
									}
								...
								if (clilocsCount != 0)
								{
									var clilocsBytes = clilocsCount * 4;

									if (fs.Position + clilocsBytes > fs.Length)
									{
										if (Core.Debug)
										{
											Console.WriteLine($"MultiData: Clilocs overflow at entry {i}/{count}, clilocs={clilocsCount}, pos={fs.Position}, length={fs.Length}");
										}

										break;
									}

									_ = fs.Seek(clilocsBytes, SeekOrigin.Current);
								}
						...
						catch (EndOfStreamException ex)
						{
							if (Core.Debug)
							{
								Console.WriteLine($"MultiData: EndOfStream reading chunkID={chunkID}, size={decompressedSize}, pos={fs.Position}");
						...
						_ = stream.Seek(positionpoint, SeekOrigin.Begin);
					}
					finally
					{
						if (sourceData != null)
		...
		private static bool LoadMUL()
		{
			...
			var entrySize = PostHSFormat ? 16 : 12;
			...
			for (var i = 0; i < count; i++)
			{
				...
				if (lookup >= 0 && length > 0)
				{
					...
					var entries = ReadEntries(mulReader, length / entrySize);
