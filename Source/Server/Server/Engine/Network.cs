	...
	public static class Compression
	{
		...
		public static bool Pack(ReadOnlySpan<byte> source, ref Span<byte> dest)
		{
			try
			{
				...
				using var comp = new ZLibStream(_CompressBuffer, CompressionMode.Compress, true);
			...
			{
				Console.WriteLine("Warning: Compression.Pack threw an exception:");
		...
		public static bool Unpack(ReadOnlySpan<byte> source, ref Span<byte> dest)
		{
			try
			{
				_DecompressBuffer ??= new MemoryStream();

				_DecompressBuffer.Position = 0;

				_DecompressBuffer.SetLength(0);

				_DecompressBuffer.Write(source);

				_DecompressBuffer.Position = 0;

				using var decomp = new ZLibStream(_DecompressBuffer, CompressionMode.Decompress, true);
			...
			{
				Console.WriteLine("Warning: Compression.Unpack threw an exception:");
				Console.WriteLine(ex);
