...
namespace Server
{
	public static class World
	{
		...
		private static List<Tuple<ConstructorInfo, string>> ReadTypes(BinaryReader tdbReader)
		{
			...
			for (var i = 0; i < count; ++i)
			{
				...
				if (t == null)
				{
					...
					{
						Console.WriteLine($"Warning: Type '{typeName}' was not found, deleting instances...");
		...
		private static void LoadInternal()
		{
			...
			{
				var deleteAll = Core.Service;

				if (!deleteAll)
				{
					Console.WriteLine("Delete all objects of that type? (y/n)");

					deleteAll = Console.ReadKey(true).Key == ConsoleKey.Y;
				}
			...
			Console.WriteLine($"Load failed (regions={failedRegions}, items={failedItems}, mobiles={failedMobiles}, guilds={failedGuilds}, type={failedType}, serial={failedSerial})");
			...
			Console.WriteLine($"Load retrying...");
